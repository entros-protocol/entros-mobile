...
import { CLIENT_PROJECTION_VERSION, SIMHASH_PUBLIC_SEED_HEX } from "./constants";

const DOMAIN = Uint8Array.from("entros-simhash-hyperplanes-v1\0", (character) =>
...
export type ProjectionPurpose = (typeof PROJECTION_PURPOSE)[keyof typeof PROJECTION_PURPOSE];
...
export function generateProjectionWords(
  ...
  while (outputIndex < wordCount) {
    ...
    for (
      let digestOffset = 0;
      digestOffset < digest.length && outputIndex < wordCount;
      digestOffset += 4
    ) {
