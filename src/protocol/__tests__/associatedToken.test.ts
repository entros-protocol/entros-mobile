...
import { deriveToken2022AssociatedAddress } from "../associatedToken";
...
describe("Token-2022 associated token address", () => {
  test.each(VECTORS)("matches the SPL Token derivation for $owner", (vector) => {
    expect(
  ...
  test("rejects an off-curve owner", () => {
    ...
    expect(() => deriveToken2022AssociatedAddress(Keypair.generate().publicKey, owner)).toThrow(
      "Associated token account owner must be on curve",
    );
  });
});
