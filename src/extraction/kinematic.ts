// PORTED VERBATIM from pulse-sdk/src/extraction/kinematic.ts.
// Kinematic features (jerk, jounce, jitter, mouse dynamics) — pure TS,
// no platform deps. Cross-platform reproducibility (Stage 3) requires
// bit-identical output between web and mobile, so this stays in sync.

import type { MotionSample, TouchSample } from "./types";
import { condense, variance, entropy } from "./statistics";

/**
 * Compute per-sample acceleration magnitude and resample to a target frame count.
 */
 ...
 * Returns: 54 values (6 axes × 2 derivatives × 4 stats + 6 jitter variances).
 */
 ...
 * Returns: 54 values (matches motion feature dimension).
 */
