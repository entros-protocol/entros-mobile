...
export function getProjectionDefinition(projectionVersion: number): ProjectionDefinition {
  if (!Number.isInteger(projectionVersion)) {
    throw new Error(`Unsupported projection version ${projectionVersion}`);
  }

  const definition =
    PROJECTION_DEFINITIONS[projectionVersion as keyof typeof PROJECTION_DEFINITIONS];
  if (!definition) {
    throw new Error(`Unsupported projection version ${projectionVersion}`);
  }
  return definition;
