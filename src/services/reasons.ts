// The verification-failure reason taxonomy, and what a caller should do about
// each one.
//
// Verbatim port of pulse-sdk/src/reasons.ts. entros-mobile does not install the
// SDK from npm, so this file is a hand-maintained mirror: the labels, their
// dispositions and the derived sets must stay identical to the web SDK. The
// SDK module exists because that list had been copied into six places with
// nothing keeping them in step, and they had already drifted. The web set
// carried more entries than mobile's, so mobile hard-failed `captcha_required`
// and dead-ended a user the web app would have let retry. Mirror the whole
// module rather than the subset mobile happens to route today, so the next
// reason to reach the executor lands here with its disposition already right.
//
...
//     attack-signal rejections and a caller must never read its absence as
//     success.
//   - The executor's own transport-level rejections: rate limits, cooldowns,
//     oversized payloads.
//   - The client itself, for failures that never reached a server.

/** Every reason label the protocol will surface. */
 ...
 *
 * The protocol forwards `reason` from the server body verbatim, so anything could
 ...
 * the executor body's `retry_after` alongside these.
 */
...
/**
 * Reasons the protocol raised itself, without a server ever seeing the capture.
 *
