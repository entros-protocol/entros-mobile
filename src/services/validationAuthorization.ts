...
const DIGEST_DOMAIN = new TextEncoder().encode("entros-validate-request-digest-v1\0");
...
export interface ClientSignals {
  v: number;
  env?: string;
  automation?: { webdriver: boolean; tells: string[] };
  capture?: { virtual_device: boolean; flatness?: number; centroid?: number };
}

export interface StudyContext {
  token: string;
  record_id: string;
  capture_class: "web-mobile" | "web-desktop" | "native-ios" | "native-android";
  feature_schema_version: number;
  projection_version: number;
}
...
class DigestEncoder {
  ...
  u32(value: number): void {
    this.assertUnsignedInteger(value, 0xffff_ffff, "u32");
  ...
  private appendNumberBytes(length: number): void {
    this.digest.update(new Uint8Array(this.numberBuffer, 0, length));
  }

  private assertUnsignedInteger(value: number, maximum: number, label: string): void {
...
export function buildValidationRequestDigest(request: ValidationDigestRequest): Uint8Array {
  ...
  if (request.audio_sample_rate_hz !== undefined) encoder.u32(request.audio_sample_rate_hz);
...
export function buildValidationAuthorizationMessage(
  ...
  if (nonce.length !== 32) throw new Error("Validation challenge nonce must be 32 bytes");
  if (digest.length !== 32) throw new Error("Validation request digest must be 32 bytes");
  if (!Number.isInteger(projectionVersion) || projectionVersion < 0 || projectionVersion > 0xffff) {
...
function studyCaptureClassCode(captureClass: StudyContext["capture_class"]): number {
  switch (captureClass) {
    case "web-mobile":
      return 0;
    case "web-desktop":
