// Wallet brand marks. Phantom and Solflare are extracted from
// @solana/wallet-adapter-* and converted to react-native-svg.
...
export const PhantomLogo = ({ size = defaultSize }: LogoProps) => (
...
// "Other wallet" tile — used for the OS-chooser path where the user can pick
// any installed MWA-compliant wallet (Seed Vault on Seeker, MetaMask, etc.).
// Neutral by design: renders a key glyph on a dark surface so it doesn't read
// as a specific brand mark. The bow and bit shapes come from Material Design's
// `vpn_key` outline, simplified for SVG path-length budget.
export const OtherWalletLogo = ({ size = defaultSize }: LogoProps) => (
  <Svg width={size} height={size} viewBox="0 0 108 108">
    <Rect width={108} height={108} rx={26} fill="#1A1A22" />
    <Circle cx={42} cy={54} r={14} stroke="#00F0FF" strokeWidth={5} fill="none" />
    <Path
      d="M53 54 H86 M77 54 V64 M86 54 V67"
      stroke="#00F0FF"
      strokeWidth={5}
      fill="none"
      strokeLinecap="round"
...
export const SolflareLogo = ({ size = defaultSize }: LogoProps) => (
