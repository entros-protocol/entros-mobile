...
import type { MotionSample, TouchSample } from "../../src/extraction/types";

export const FINGERPRINT_ARCHITECTURE_FIXTURE_ID = "entros-fingerprint-architecture-fixture-v1";
...
export const EXPECTED_FINGERPRINT_ARCHITECTURE_OUTPUTS = [
  {
    projectionVersion: 0,
    fingerprintHex: "fa8ef7bad393171a094feb86699765de35ce65ca59b8de15e76a900ab76de86c",
    commitmentHex: "0ce1b185d50e89cc0b5f2e4682cbf0e8020a0bff1b85302f9c394cf007ff069e",
  },
  {
    projectionVersion: 1,
    fingerprintHex: "234281cb32d28fb297e9ea54806a8e61a8be00f095d0df76a9f132cc1ff1105c",
    commitmentHex: "2582455281bbc908f1f5fa863fa42e961971127ac005187f3796c9dcd03892cb",
  },
  {
    projectionVersion: 2,
    fingerprintHex: "2d9c86198251b94e9285a3b4ae87667b52363261ed0eb523bc9877b4a1a63a15",
    commitmentHex: "0e1fe84296ee186cf1bf7ae8b1628d1fefa01b8ddb1a4f9eac43b69874889e9f",
  },
] as const;
...
function createSourcePcm(): Float32Array {
  const samples = new Float32Array(FINGERPRINT_ARCHITECTURE_SOURCE_SAMPLE_COUNT);
  ...
  for (let index = 0; index < samples.length; index++) {
    ...
    const integerSample =
      ((triangleA << 1) - 128) * 76 + ((triangleB << 1) - 256) * 18 + (noise << 2);
...
function createMotion(): MotionSample[] {
  const samples = new Array<MotionSample>(FINGERPRINT_ARCHITECTURE_SENSOR_SAMPLE_COUNT);
  for (let index = 0; index < samples.length; index++) {
    samples[index] = {
...
function createTouch(): TouchSample[] {
  const samples = new Array<TouchSample>(FINGERPRINT_ARCHITECTURE_SENSOR_SAMPLE_COUNT);
  for (let index = 0; index < samples.length; index++) {
    samples[index] = {
      timestamp: (index * 125) / 8,
...
export function encodeFingerprintArchitectureFixture(
  ...
  const domain = new TextEncoder().encode(`${FINGERPRINT_ARCHITECTURE_FIXTURE_ID}\0`);
  const byteLength =
    domain.length +
    4 +
    4 +
